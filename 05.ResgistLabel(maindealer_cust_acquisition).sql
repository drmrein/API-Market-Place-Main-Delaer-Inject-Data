...
BEGIN
	SELECT @flagGroup_H		= 1						-- perlu add group H ? jika true, wajib mengisi variable @GroupName
		  ...
		  ,@alias			= '_MAIND_CUST_ACQ'		-- diawali dengan underscore _ untuk penamaan Question Identifier
...
/*setting Group H*/
SET @GroupName = 'maindealer_cust_acquisition' -- nama group baru 
...
BEGIN 
	...
	SELECT REPLACE('ID_NAME              ',' ','')QuestionCode    , REPLACE('idName            ',' ','')QuestionApi  INTO #tempLabelCode  UNION
	SELECT REPLACE('ORDER_ID             ',' ','')    , REPLACE('orderIdDealer     ',' ','') UNION
	SELECT REPLACE('PARTNER_ID           ',' ','')    , REPLACE('dealerId          ',' ','') UNION
	SELECT REPLACE('ID_NO                ',' ','')    , REPLACE('idNo              ',' ','') UNION
	SELECT REPLACE('FULL_NAME            ',' ','')    , REPLACE('fullName          ',' ','') UNION
	SELECT REPLACE('BIRTH_PLACE          ',' ','')    , REPLACE('birthPlace        ',' ','') UNION
	SELECT REPLACE('BIRTH_DT             ',' ','')    , REPLACE('birthDate         ',' ','') UNION
	SELECT REPLACE('SURVEY_ADDR          ',' ','')    , REPLACE('surveyAddr        ',' ','') UNION
	SELECT REPLACE('SURVEY_RT            ',' ','')    , REPLACE('surveyRT          ',' ','') UNION
	SELECT REPLACE('SURVEY_RW            ',' ','')    , REPLACE('surveyRW          ',' ','') UNION
	SELECT REPLACE('SURVEY_PROVINCE      ',' ','')    , REPLACE('surveyProvince    ',' ','') UNION
	SELECT REPLACE('SURVEY_CITY          ',' ','')    , REPLACE('surveyCity        ',' ','') UNION
	SELECT REPLACE('SURVEY_SUB_DISTRICT  ',' ','')    , REPLACE('surveySubDistrict ',' ','') UNION
	SELECT REPLACE('SURVEY_VILLAGE       ',' ','')    , REPLACE('surveyVillage     ',' ','') UNION
	SELECT REPLACE('SURVEY_CITY_ID       ',' ','')    , REPLACE('surveyZipcode     ',' ','') UNION
	SELECT REPLACE('ADDR                 ',' ','')    , REPLACE('legalAddr         ',' ','') UNION
	SELECT REPLACE('RT                   ',' ','')    , REPLACE('legalRt           ',' ','') UNION
	SELECT REPLACE('RW                   ',' ','')    , REPLACE('legalRw           ',' ','') UNION
	SELECT REPLACE('PROVINCE             ',' ','')    , REPLACE('legalProvince     ',' ','') UNION
	SELECT REPLACE('CITY                 ',' ','')    , REPLACE('legalCity         ',' ','') UNION
	SELECT REPLACE('SUB_DISTRICT         ',' ','')    , REPLACE('legalSubDistrict  ',' ','') UNION
	SELECT REPLACE('VILLAGE              ',' ','')    , REPLACE('legalVilage       ',' ','') UNION
	SELECT REPLACE('CITY_ID              ',' ','')    , REPLACE('legalZipcode      ',' ','') UNION
	SELECT REPLACE('PHONE_NUMBER         ',' ','')    , REPLACE('mobilePhone       ',' ','') UNION
	SELECT REPLACE('MOTHER_MAIDEN_NAME   ',' ','')    , REPLACE('motherName        ',' ','')
END
