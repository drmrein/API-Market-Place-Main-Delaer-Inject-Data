...
BEGIN
	SELECT @flagGroup_H		= 1						-- perlu add group H ? jika true, wajib mengisi variable @GroupName
		  ...
		  ,@alias			= '_UPD_STATUS_MSS'		-- diawali dengan underscore _ untuk penamaan Question Identifier
...
/*setting Group H*/
SET @GroupName = 'maindealer_upd_status_mss' -- nama group baru 
...
BEGIN 
	...
	SELECT REPLACE('ID_NAME',' ','')QuestionCode    , REPLACE('idName',' ','')QuestionApi,1 seq  INTO #tempLabelCode  UNION
	SELECT REPLACE('ORDER_ID',' ','')    , REPLACE('orderId',' ',''),2 UNION
	SELECT REPLACE('STATUS_MSS_WISE',' ','')    , REPLACE('statusMss',' ','') ,3
END
