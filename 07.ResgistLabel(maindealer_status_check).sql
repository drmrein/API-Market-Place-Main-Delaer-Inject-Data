...
BEGIN
	SELECT @flagGroup_H		= 1						-- perlu add group H ? jika true, wajib mengisi variable @GroupName
		  ...
		  ,@alias			= '_MAINDLR_STAT_CHK'	-- diawali dengan underscore _ untuk penamaan Question Identifier
...
/*setting Group H*/
SET @GroupName = 'maindealer_status_check' -- nama group baru 
...
BEGIN 
	SELECT REPLACE('PARTNER_ID                ',' ','') QuestionCode   , REPLACE('delaerID        ',' ','') QuestionApi INTO #tempLabelCode UNION
	SELECT REPLACE('ID_NAME                   ',' ','')    , REPLACE('idName          ',' ','') UNION
	SELECT REPLACE('ORDER_ID                  ',' ','')    , REPLACE('orderIdDealer          ',' ','')  
END
